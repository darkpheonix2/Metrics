...
This repository contains a Jupyter notebook, `Analysis.ipynb`, used to explore and analyze a collection of evaluation metrics for question answering / LLM responses. The analysis operates on `Input_data/Testing_dataset.csv` and produces an `evaluation_results.csv` file containing model responses, reference answers, contextual fields, and a wide range of automatic metrics (e.g. token-based, BLEU/METEOR, BERTScore, NLI-style scores, embedding-based metrics, retrieval metrics, and ROUGE).

### Repository structure

- **`Input_data/Testing_dataset.csv`**: The synthetic dataset created after manual filtration from 9000 synthetically generated samples using Llama-3.2-3B-Instruct.
...
- **`evaluation_results.csv`** (expected): Input data file with columns including:
  ...
  - Standard NLP metrics: `EM`, `Token_F1`, `Token_Precision`, `Token_Recall`, `Token_Accuracy`, `Fuzzy_Accuracy`, `LCS`, `Jaccard_Similarity`, `ROUGE_1`, `ROUGE_2`, `ROUGE_L`, `METEOR`, `BLEU.`
  ...
  - Retrieval metrics: `Faithfulness`, `Context_Precision`, `Context_Recall`, `Retrieval_Precision`, `Retrieval_Recall`, `Retrieval_F1`, `Retrieval_TP`, `Retrieval_FP`, `Retrieval_FN`, `Answer_Relevance`, `NLI_Precision`, `NLI_Recall`, `NLI_F1`, `GEval_Precision`, `GEval_Recall`, `GEval_F1`, `Is_Error`
...
2. **Ensure the data file is available**:
   - Place `Testing_dataset.csv` in the root of this repo (alongside `Analysis.ipynb`), or adjust the path in the first data-loading cell of the notebook.
